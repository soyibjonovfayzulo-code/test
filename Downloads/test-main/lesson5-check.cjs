...
(async () => {
  ...
  click(bh.querySelector('[data-gap="0"]'));
  await sleep(50);
  ok(doc.querySelector('[data-game-body="bughunter"]').textContent.includes('Bug Hunter tugadi'), 'Bug Hunter: 3/3 round — g\'alaba');
  ...
  /* Memory o'yinini TO'LIQ yakunlash (unlock zanjiri: 3 → 4) */
  {
    ...
    ok(doc.querySelector('[data-game-body="memory"]').textContent.includes('Memory Code tugadi'), 'Memory: tugadi ekrani');
  ...
  await sleep(50);
  ok(doc.querySelector('[data-game-body="runner"]').textContent.includes('FINISH'), 'Code Runner: finish chizig\'iga yetdi');
  ok(doc.querySelector('[data-game-body="runner"]').closest('.ls-game-card').querySelector('[data-game-badge="runner"]').textContent.includes('Bajarildi'), 'Code Runner badge ✅');
  ...
  ok(doc.querySelector('[data-game-body="streak"]').textContent.includes('STREAK FIRE'), 'Streak Fire: 7 streak — g\'alaba');
  ok(doc.querySelector('[data-game-body="streak"]').closest('.ls-game-card').querySelector('[data-game-badge="streak"]').textContent.includes('Bajarildi'), 'Streak badge ✅');
  ...
  await sleep(950);
  ok(doc.querySelector('[data-game-body="duel"]').textContent.includes('G‘ALIB'), 'Code Duel: 3/3 to\'g\'ri — g\'alaba');
  ...
  /* ✅ Bajarildi o'yinlar qayta ochilsa: ✅ Bajarildi + 🔄 Qayta o'ynash (yagona replay kontrakt) */
  const doneCards = Array.from(doc.querySelectorAll('.ls-game-card.done'));
  ok(doneCards.length === 7, 'Darsga qaytish: 7 ta o\'yin ✅ Bajarildi holatda (' + doneCards.length + ')');
  const replayBtns = doc.querySelectorAll('.ls-game-replay-btn');
  ok(replayBtns.length === 7, 'Har bir bajarilgan o\'yinda 🔄 Qayta o\'ynash tugmasi bor (' + replayBtns.length + ')');
  ok(doneCards[0].textContent.includes('Bajarildi') && doneCards[0].textContent.includes('Qayta o‘ynash'), 'Bajarilgan karta: ✅ Bajarildi + 🔄 Qayta o‘ynash');
  /* REPLAY: birinchi o'yinni qayta o'ynash — keyingi o'yinlar lock/unlock holati o'zgarmasligi kerak */
  const lockedBefore = doc.querySelectorAll('.ls-game-card.locked').length;
  click(replayBtns[0]);
  await sleep(20);
  const bhBody = doc.querySelector('[data-game-body="bughunter"]');
  ok(bhBody && bhBody.querySelectorAll('.ls-gh-gap').length >= 3, 'Replay: Bug Hunter boshidan boshlandi (toza attempt)');
  ok(doc.querySelectorAll('.ls-game-card.locked').length === lockedBefore, 'Replay: o\'yinlar unlock holati o\'zgarmadi');
  ok(doneCards.every(function (c) { return c.textContent.includes('Bajarildi'); }), 'Replay: barcha o\'yinlar ✅ Bajarildi holatida qoldi');
